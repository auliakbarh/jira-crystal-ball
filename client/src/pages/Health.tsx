    ...
    <div className="mx-auto max-w-md px-4 py-10">
      <div className="card space-y-4">
        ...
        <div className="space-y-2">
          <Row label={t("health.graphqlApi")} ok={apiUp} hint={error ? error.message : undefined} />
          <Row label={t("health.database")} ok={!!h?.database} />
          <Row
            label={t("health.jiraCredentials")}
            ok={!!h?.jira}
            hint={h && !h.jira ? t("health.jiraEnvNotSet") : undefined}
          />
        </div>
        ...
        >
          {apiUp && h?.ok ? t("health.operational") : t("health.degraded")}
        </div>

        {h?.time && (
          <div className="text-center text-xs text-gray-400">
            {t("health.serverTime")}: {new Date(h.time).toLocaleString()}
          </div>
        )}
